...
class GaPageviewsPerSession extends GoogleAnalyticsApiBase implements IManagerKPI {
    ...
    function doExecute($date) {
        try {
            ...
            //取得するデータの組み合わせ
            $metrics = "ga:pageviewsPerSession";  //メトリクスの設定

            //データの取得
            $obj = $this->get($this->view_id, $from, $to, $metrics);
            list($result) = $obj['rows'];

            return $result[0] ? round($result[0],2) : 0;
        } catch (Exception $e) {
