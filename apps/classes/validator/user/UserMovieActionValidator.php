...
class UserMovieActionValidator extends BaseValidator {
    ...
    public function validate()
    {
        ...
        // キャンペーンのステータスチェック
        if ($cp->status != Cp::STATUS_DEMO && $cp->status != Cp::STATUS_FIX) {
            $this->errors['cp_id'][] = "キャンペーンが存在しません";
            return;
        }
        ...
        if (!$cp_user_action_status->id) {
            aafwLog4phpLogger::getHipChatLogger()->info('UserMovieActionValidator');
            $this->errors['cp_action_id'][] = "アクションが存在しません";
            return;
        }
