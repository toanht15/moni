...
<article>
    ...
    <form id="addCategoryForm" name="addCategoryForm" action="<?php  assign(Util::rewriteUrl( 'admin-blog', 'create_static_html_category' )); ?>" method="POST" enctype="multipart/form-data">
    ...
    <dl class="categoryEdit1">
        ...
        <dd><?php write_html('<a id="folder_name">' . Util::constructBaseURL($data['pageStatus']['brand']->id, $data['pageStatus']['brand']->directory_name) . 'categories/</a>'.$this->formText('directory', PHPParser::ACTION_FORM, array('class'=>'directoryName', 'id'=>'field2', 'maxlength'=>35))) ?></dd>
        <p><small>※未設定の場合は自動的にカテゴリー名が割り振られます。</small></p>
        <?php if ($this->ActionError && !$this->ActionError->isValid('directory')): ?>
            <p class="attention1"><?php assign($this->ActionError->getMessage('directory')) ?></p>
        <?php endif; ?>
        <dt>親カテゴリ</dt>
        <dd>
        ...
        <dd class="jsCheckToggleWrap">
            ...
            <div class="viewCustom jsCheckToggleTarget">
                ...
                <?php if ($this->ActionError && !$this->ActionError->isValid('customize_code')): ?>
                    <p class="attention1"><?php assign($this->ActionError->getMessage('customize_code')) ?></p>
                <?php endif; ?>
                %title%: カテゴリーのタイトル</br>
            ...
            <dl>
                ...
                <dd>
                    ...
                    <?php if ($this->ActionError && !$this->ActionError->isValid('description')): ?>
                        <p class="attention1"><?php assign($this->ActionError->getMessage('description')) ?></p>
                    <?php endif; ?>
                </dd>
                ...
                <dd>
                    ...
                    <?php if ($this->ActionError && !$this->ActionError->isValid('keyword')): ?>
                        <p class="attention1"><?php assign($this->ActionError->getMessage('keyword')) ?></p>
                    <?php endif; ?>
                </dd>
                <dt>OG Image</dt>
                <dd>
