...
<article>
    ...
    <div class="customaudienceWrap">
        <form name="save_audience_form" id="updateAudience" method="POST" action="<?php assign(Util::rewriteUrl("admin-fan", "update_ads_audience")) ?>">
            <?php write_html($this->formHidden('audience_id', $data["audience_id"])) ?>
            ...
            <dl class="customaudienceDataSetting">
                ...
                <dt class="require1"><label>タイトル</label>
                    <span class="iconHelp">
                        ...
                        <span class="textBalloon1">
                          <span>タイトルを変更すると、<br>Facebookビジネスマネジャーの<br>オーディエンス名も更新されます</span>
                        <!-- /.textBalloon1 --></span>
                    <!-- /.iconHelp --></span>
                </dt>
                <dd>
                    <?php write_html($this->formText('audience_name', PHPParser::ACTION_FORM, array('placeholder' => 'カスタムオーディエンスのタイトル', 'maxlength' => 255, 'class' => 'jsNameInput'))) ?>
                    <?php if ( $this->ActionError && !$this->ActionError->isValid('audience_name')): ?>
                        <span class="iconError1 jsNameInputError"><?php assign ( $this->ActionError->getMessage('audience_name') )?></span>
                    <?php endif; ?>
                ...
                <dt>送信対象</dt>
                <dd class="spreadWidth">
                    <?php if($data['audience']->search_type == AdsAudience::SEACH_TYPE_SEGMENT):?>

                        <?php write_html(aafwWidgets::getInstance()->loadWidget('SegmentAdsTargetList')->render(array('provision_ids' => $data['provision_ids']))) ?>

                    <?php elseif($data['audience']->search_type == AdsAudience::SEACH_TYPE_ADS): ?>
...
<?php write_html($this->parseTemplate('ads/AddAdsAccountModal.php', array('callback_url' => Util::rewriteUrl('admin-fan','ads_audience', array('audience_id' => $data["audience_id"]))))) ?>
