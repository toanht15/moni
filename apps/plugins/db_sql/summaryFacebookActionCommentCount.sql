SELECT BSA.social_media_account_id, SUM(FBEUC.cmt_count) action_count, BSA.social_app_id, SA.user_id uid, DCU.data_type log_type
...
INNER JOIN fb_entries_users_comments FBEUC ON FBEUC.object_id = DCU.object_id AND FBEUC.del_flg = 0
INNER JOIN social_accounts SA ON SA.social_media_account_id = FBEUC.fb_uid AND SA.social_media_id = ?social_media_id? AND SA.del_flg = 0
WHERE BSA.del_flg = 0 AND BSA.social_app_id = ?social_app_id? AND BSA.hidden_flg = 0
GROUP BY FBEUC.fb_uid,BSA.id