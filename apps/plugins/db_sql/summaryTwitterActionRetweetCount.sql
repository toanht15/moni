...
INNER JOIN tw_entries_users_retweets TEUR ON TEUR.entry_object_id = DCU.object_id
INNER JOIN social_accounts SA ON SA.social_media_account_id = TEUR.tw_uid AND SA.social_media_id = ?social_media_id? AND SA.del_flg = 0
WHERE BSA.del_flg = 0 AND BSA.social_app_id = ?social_app_id? AND BSA.hidden_flg = 0
GROUP BY TEUR.tw_uid,BSA.id
