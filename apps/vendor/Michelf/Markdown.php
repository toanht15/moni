...
class Markdown implements MarkdownInterface {
    ...
    public static function defaultTransform($text) {
        ...
        $text = $parser->parseMarkdownUrl($text);
     ...
     * @return mixed|string
     */
    public function parseMarkdownUrl($text) {
        #
        # Parsing url
        #
...
//        "encodeAmpsAndAngles" =>  40,
			...
			(				# wrap whole match in $1
			  ...
			  \(			# literal paren
				...
				(			# $8
				  ([_])		# "_" char = $9
				  (.*?)		# target = $10
				  [ \n]*	# ignore any spaces/tabs between target $10 and ")"
				)?			# target is optional
			  \)
			)
			}xs',
    ...
    protected function _doImages_inline_callback($matches) {
        ...
        if (isset($size)) {
            $img_sizes = explode("-", $size);

            $width = $this->encodeAttribute($img_sizes[0]);
            $height = $this->encodeAttribute($img_sizes[1]);
...
//				}
    ...
    protected function doAutoLinks($text) {
        preg_match_all('{<((https?|ftp|dict|tel):[^\'">\s]+)>}i', $text, $temp);
