    ...
    <table class="itemTable">
        ...
        <tr>
            ...
            <th class="jsAreaToggleWrap" colspan="1">
                <?php assign(Util::cutTextByWidth($data['cp_twitter_follow_action']->title, 750)); ?>
            </th>
        </tr>
        <tr>
            <th class="jsAreaToggleWrap" title="Twitterフォロー">
                Twitterフォロー状況
                <a href="javascript:void(0)" class="<?php assign($data['search_condition'][CpCreateSqlService::SEARCH_TW_FOLLOW_TYPE . '/' . $data['cp_twitter_follow_action']->cp_action_id] ? 'iconBtnSort' : 'btnArrowB1'); ?> jsAreaToggle">絞り込む</a>
                <?php write_html($this->parseTemplate('SearchTwFollowType.php', array(
                    'search_tw_follow' => $data['search_condition'][CpCreateSqlService::SEARCH_TW_FOLLOW_TYPE . '/' . $data['cp_twitter_follow_action']->cp_action_id],
                    'action_id' => $data['cp_twitter_follow_action']->cp_action_id))) ?>
            </th>
        </tr>
        </thead>
        <tbody>
        <?php foreach($data['fan_list_users'] as $fan_list_user): ?>
            ...
            <?php endif; ?>
                ...
                <td><?php assign($data['cp_twitter_follow_log_list'][$fan_list_user->cp_user_id]->status_string ? : '') ?></td>
            </tr>
        <?php endforeach; ?>
