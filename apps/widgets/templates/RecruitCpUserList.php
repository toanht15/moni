        ...
        <ul class="userName">
            <form id="formAddTarget" name="formAddTarget" action="<?php assign(Util::rewriteUrl('admin-cp', 'update_fan_target.json')); ?>" method="POST">
                ...
                <?php foreach($data['fan_list_users'] as $fan_list_user): ?>
                    ...
                    <?php if($page_user_message[0]): ?>
                        <label title="<?php assign($answer) ?>"><input type="checkbox" class="jsCountTarget" name="user[]" value="<?php assign($fan_list_user->user_id) ?>" checked disabled><img src="<?php assign($user_info->profile_image_url ? $user_info->profile_image_url : config('Static.Url').'/img/base/imgUser1.jpg') ?>" width="20" height="20" alt="<?php assign(Util::cutTextByWidth($answer, 102)) ?>" onerror="this.src='<?php assign($this->setVersion('/img/base/imgUser1.jpg'));?>';"><?php assign(Util::cutTextByWidth($answer, 102)) ?></label>
                    <?php else: ?>
                        <label title="<?php assign($answer) ?>"><input type="checkbox" class="jsCountTarget" name="user[]" value="<?php assign($fan_list_user->user_id) ?>" <?php assign($data['cp']->status == Cp::STATUS_SCHEDULE && $data['cp']->send_mail_flg == Cp::FLAG_SHOW_VALUE ? 'disabled' : '')?>><img src="<?php assign($user_info->profile_image_url ? $user_info->profile_image_url : config('Static.Url').'/img/base/imgUser1.jpg') ?>" width="20" height="20" alt="<?php assign(Util::cutTextByWidth($answer, 102)) ?>" onerror="this.src='<?php assign($this->setVersion('/img/base/imgUser1.jpg'));?>';"><?php assign(Util::cutTextByWidth($answer, 102)) ?></label>
                    <?php endif; ?>
                    </li>
                <?php endforeach; ?>
                <?php write_html($this->csrf_tag()); ?>
