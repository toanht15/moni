<section class="jsPanel contBoxMain <?php  assign($data['panel']['entry']['size_class'])?> <?php if ($data['panel']['panelType'] == BrandcoTopMainCol::PANEL_TYPE_TOP && $data['isLoginAdmin']) assign(' contFixed')?>">
    <a href="<?php assign($data['panel']['entry']["link"])?>" class="contInner panelClick" <?php if ($data['panel']['entry']['target'] == LinkEntry::TARGET_BLANK) write_html('target="_blank"') ?>
       ...
       data-entry="<?php assign(StreamService::STREAM_TYPE_LINK)?>"
       data-entry_id="<?php assign($data['panel']['entry']["id"])?>">
        <div class="contWrap">
            ...
            <?php if($data['panel']['entry']["body"]): ?>
                <p class="contText">
                    <span><?php write_html($this->nl2brAndHtmlspecialchars($data['panel']['entry']["body"]))?></span>
                </p>
            <?php endif; ?>
            <p class="postType"><?php assign($data['panel']['entry']["title"])?></p>
        </div>
    </a>
    <?php if($data['isLoginAdmin']):?>
    <ul class="editBox1">
        <li><a href="#editLIPanelForm" data-option=<?php assign('/'.$data['panel']['entry']["id"].'?from=top')?> class="linkEdit jsOpenModal">編集</a></li>
